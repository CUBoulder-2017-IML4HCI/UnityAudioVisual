...
void setup()
{
  ...
  oscP5 = new OscP5(this,8888); //listen for OSC messages on port 12000 (Wekinator default)
  dest = new NetAddress("127.0.0.1",9999); //send messages back to Wekinator on port 6448, localhost (this machine) (default)
...
void draw()
{
  ...
  fill(0, 255, 0);
  text( "Use 3 continuous Unity outputs between 0 and 1", 5, 15 );
  text( "Listening for /music on port 8888", 5, 30 );
}
...
void oscEvent(OscMessage theOscMessage) {
 ...
 if (theOscMessage.checkAddrPattern("/music")==true) {
     if(theOscMessage.checkTypetag("ffff")) { //Now looking for 2 parameters
        ...
        float p4 = theOscMessage.get(3).floatValue(); //get fourth parameter 4 pitch
        updateDrums(p1, p2, p3);
        
        myMessage = new OscMessage("/scale");
        myMessage.add(p4); /* add an int to the osc message */      
